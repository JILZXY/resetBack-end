...
async function runApiTests() {
    ...
    try {
        ...
        logResult('--- Starting Comprehensive API Tests via Selenium (Final Run) ---');

        logResult(`Navigating to ${baseUrl}...`);
        await driver.get(baseUrl);
        await driver.sleep(1000);

        async function testEndpoint(name: string, endpointPath: string, method: string = 'GET', body: any = null) {
            testCount++;
            ...
            try {
                ...
                if (result.status >= 200 && result.status < 300) {
                    logResult(`✅ SUCCESS: ${method} ${endpointPath} returned ${result.status}`);
                    successCount++;
                    return result.data;
                } else {
                    const errorDetails = result.error || JSON.stringify(result.data);
                    logResult(`❌ FAILED: ${method} ${endpointPath} returned ${result.status}. Detail: ${errorDetails}`);
                    return result;
                }
            } catch (err: any) {
                logResult(`❌ ERROR: ${method} ${endpointPath} threw an exception: ${err.message}`);
            }
            return null;
        }
        ...
        // 2. Login
        const loginRes = await testEndpoint('Login', '/auth/login', 'POST', { email, password });
        ...
        if (loginData && (loginData.accessToken || loginData.data?.accessToken)) {
            ...
            const fallbackRes = await testEndpoint('Login Fallback', '/auth/login', 'POST', {
                email: "fullgamer494@hotmail.com",
                password: "qwerty123"
            });
            let fallbackData = fallbackRes?.data || fallbackRes;
            if (fallbackData && (fallbackData.accessToken || fallbackData.data?.accessToken)) {
        ...
        const contactRes = await testEndpoint('Add Contact', '/emergency/contacts', 'POST', {
        ...
        if (contactId) {
            await testEndpoint('Delete Contact', `/emergency/contacts/${contactId}`, 'DELETE');
        }
        ...
        const postRes = await testEndpoint('Create Forum Post', '/forum/posts', 'POST', {
        ...
        if (postId) {
            ...
            await testEndpoint('React to Post', `/forum/posts/${postId}/react`, 'POST', { type: "like" });
            await testEndpoint('Get Comments', `/forum/posts/${postId}/comments`);
            const commentRes = await testEndpoint('Add Comment', `/forum/posts/${postId}/comments`, 'POST', {
            ...
            if (commentId) {
                await testEndpoint('Delete Comment', `/forum/comments/${commentId}`, 'DELETE');
            }
