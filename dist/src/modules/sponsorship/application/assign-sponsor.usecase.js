...
let RequestSponsorshipUseCase = class RequestSponsorshipUseCase {
    ...
    async execute(userId, dto) {
        ...
        if (!sponsor || sponsor.role !== 'PADRINO') {
            throw new common_1.HttpException({ code: 'SPONSOR_NOT_FOUND', message: 'Código de padrino no válido o no encontrado', details: {} }, common_1.HttpStatus.NOT_FOUND);
        }
        if (sponsor.id === userId) {
            throw new common_1.HttpException({ code: 'SELF_SPONSORSHIP', message: 'No puedes ser tu propio padrino', details: {} }, common_1.HttpStatus.BAD_REQUEST);
        }
        const existingActive = await this.sponsorshipRepo.findActiveByAddictId(userId);
        if (existingActive) {
            throw new common_1.HttpException({ code: 'ALREADY_HAS_SPONSOR', message: 'Ya tienes un padrino activo', details: {} }, common_1.HttpStatus.CONFLICT);
        }
        const existingPending = await this.sponsorshipRepo.findPendingByAddictId(userId);
        if (existingPending) {
            throw new common_1.HttpException({ code: 'PENDING_REQUEST_EXISTS', message: 'Ya tienes una solicitud de apadrinamiento pendiente', details: {} }, common_1.HttpStatus.CONFLICT);
        }
