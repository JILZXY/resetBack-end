...
let DeleteCommentUseCase = class DeleteCommentUseCase {
    ...
    async execute(userId, commentId) {
        ...
        if (!comment) {
            throw new common_1.HttpException({ code: 'COMMENT_NOT_FOUND', message: 'Comentario no encontrado', details: { commentId } }, common_1.HttpStatus.NOT_FOUND);
        }
        if (comment.authorId !== userId) {
            throw new common_1.HttpException({ code: 'FORBIDDEN', message: 'No tienes permiso para eliminar este comentario', details: {} }, common_1.HttpStatus.FORBIDDEN);
        }
        await Promise.all([
