...
export class RequestSponsorshipUseCase {
  ...
  async execute(userId: string, dto: RequestSponsorshipDto) {
    ...
    if (!sponsor || sponsor.role !== 'PADRINO') {
      throw new HttpException(
        { code: 'SPONSOR_NOT_FOUND', message: 'Código de padrino no válido o no encontrado', details: {} },
    ...
    if (sponsor.id === userId) {
      throw new HttpException(
        { code: 'SELF_SPONSORSHIP', message: 'No puedes ser tu propio padrino', details: {} },
    ...
    if (existingActive) {
      throw new HttpException(
        { code: 'ALREADY_HAS_SPONSOR', message: 'Ya tienes un padrino activo', details: {} },
        HttpStatus.CONFLICT,
      );
    }
    ...
    if (existingPending) {
      throw new HttpException(
        { code: 'PENDING_REQUEST_EXISTS', message: 'Ya tienes una solicitud de apadrinamiento pendiente', details: {} },
        HttpStatus.CONFLICT,
      );
    }
  ...
  private async notifySponsorship(actorId: string, sponsorId: string, sponsorEmail: string) {
    // Notificación in-app
